...
chaturadissanayake.com/
  ...
  assets/
    css/
      ...
      journal.css         Theme override for notes.html only
    ...
    off-duty/               Images used only by notes.html
...
These exist only as entries in `data/projects.json`, with `link` pointing to an external URL (their own domain, GitHub Pages, UN pages, or a Drive folder). They should **not** appear in `sitemap.xml` – this is not their canonical domain:
